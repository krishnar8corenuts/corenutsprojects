...
export class AddPaperComponent implements OnInit {
  ...
  getquestions(id:any)
  {
    if(this.selectedsubject != 'CODING'){
      this.http.get<Question[]>(`http://34.138.180.59:8089/api/getallquestions/${id}`).subscribe(data=>{
      console.log(data);
...
addpaper(createPaper:CreatePaper)
{
  ...
  if (this.noOfQuestions && this.questionCount < this.noOfQuestions){
    ...
    this.http.post<CreatePaper>(`http://34.138.180.59:8089/api/addpaper`,createPaper).subscribe(
      data=>{
