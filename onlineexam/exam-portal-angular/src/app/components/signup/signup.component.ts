...
export class SignupComponent {
  ...
  registerNewUser(newUser: User) {
    ...
    if (this.submitted) {
      this.http.post(`http://34.138.180.59:8088/api/registerUser`, newUser).subscribe(data => {
        if (data != null) {
          swal.fire("Registered successfully", 'User Name is ' + this.userLogin.email, "success");
          this.router.navigateByUrl('/login');
        } else {
          swal.fire("Already have an Account", "", "error");
        }
      });
    }
  }
