...
export class UserAnswersComponent implements OnInit {
  ...
  loadQuestions(): Observable<Question[]> {
    return this.http.get<Question[]>(`http://34.138.180.59:8089/api/getquestionsBySubjectId/${this.code}`);
  }



  loadScore(): Observable<number>{
    return this.http.get<number>(`http://34.138.180.59:8090/api/getScore/${this.uid}/${this.eid}`);
  }

  loadUserAnswers(): Observable<useranswer[]>{
    return this.http.get<useranswer[]>(`http://34.138.180.59:8089/api/getUserAnswers/${this.uid}/${this.eid}`);
  }

  loadExam():Observable<ScheduleExam>{
    return this.http.get<ScheduleExam>(`http://34.138.180.59:8089/api/getexam/${this.eid}`)
  }
  saveScore(){
    this.http.post(`http://34.138.180.59:8089/api/savemarks`,this.obtainedMarks).subscribe((data)=>console.log(data+"saved"));
  }
