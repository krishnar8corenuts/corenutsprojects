...
export class UserexamComponent {
  ...
  ngOnInit(): void {
    ...
    this.startTimer()
    this.http.get(`http://34.138.180.59:8089/api/getquestions/${this.eid}`).subscribe(data=>{this.examtime=data
  ...
  if(examtime){
    ...
    this.route.params.subscribe((params) => {
      ...
      this.http.get<any>(`http://34.138.180.59:8089/api/getquestionsBySubjectId/${this.code}`).subscribe(data=>{this.questions=data,
...
startTimer() {
  ...
  loadSubjects(): Observable<Subject[]> {
    return this.http.post<Subject[]>(
      `http://34.138.180.59:8089/api/getsubjectsBycode/${this.code}`,
      this.code
    );
  }
  ...
  loadQuestions(subjectid?: number) {
    ...
    return this.http.get(
      `http://34.138.180.59:8089/api/getquestionsBySubjectId/${subjectid}/${this.code}`
    );
  ...
  sendoption(qid:number,option1:string)
  {
    ...
    this.answer = {
      ...
      userAnswer: option1,
       ...
       this.http.post(`http://34.138.180.59:8089/api/saveanswer`,this.answer).subscribe(data=>{
       console.log("188");
...
clickEvent(exam: any) {
  ...
  if(remainingQuestion>0){
    swal.fire({
      ...
      text: "Remaining questions are "+ remainingQuestion + " , Are you sure you want to Submit?",
